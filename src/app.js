...
import { rotateCamera, onHover, onClick } from "./resources/utility.js";
...
import { addText, addPhotoOnPlane } from "./resources/surfaces.js";
...
function start() {
  ...
  addText(
    ...
    0.01,
    -40,
    "※ No need for full name and profile pic URL to login",
    2,
    0x292929
  );
  ...
  document
    ...
    .addEventListener("click", loadingPageEventListener);
...
function createBar() {
  for (var i = 0; i < bar_info.length; i += 1) {
    ...
    transform.setOrigin(
      new Ammo.btVector3(graph.position.x, 0, graph.position.z)
    );
